<template>
  ...
  >
    <div 
      class="bg-white dark:bg-slate-800 rounded-lg max-w-md w-full p-6 relative shadow-xl animate-modal-open"
    >
      <button 
        @click="$emit('close')" 
        class="absolute top-4 right-4 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 text-2xl hover:bg-indigo-100 dark:hover:bg-slate-800 rounded-full p-2 transition h-10 w-10 flex items-center justify-center"
      >
        ✕
      </button>
      ...
      <div 
        v-if="equivalentPurchases.length" 
        class="space-y-3 mb-6"
        :class="{ 'animate-fade-in': showPurchases }"
        v-show="showPurchases"
      >
        <div class="flex justify-between items-center mb-2">
          <span class="text-slate-600 dark:text-slate-300">Here's what you missed out on:</span>
          <button 
            @click="$emit('reloadPurchases')" 
            class="text-indigo-500 dark:text-indigo-400 hover:text-indigo-600 dark:hover:text-indigo-300 p-1 rounded-full hover:bg-indigo-50 dark:hover:bg-slate-800 transition"
          >
            🔄
          </button>
        </div>
        <div 
          ...
          :key="purchase.name"
          class="flex items-center space-x-2 text-slate-800 dark:text-slate-100"
        >
      ...
      <!-- <button 
        @click="shareResults"
        class="bg-indigo-500 text-white w-full py-2 rounded-lg hover:bg-indigo-600 dark:bg-indigo-600 dark:hover:bg-indigo-700 transition"
      >
        Share My Pain
      </button> -->
    </div>
